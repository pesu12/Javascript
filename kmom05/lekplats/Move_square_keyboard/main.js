...
Player.prototype.stayInArea = function(width, height) {
  if(this.position.y < -this.height+32)  this.position.y = 0;
  if(this.position.y > height-33)        this.position.y = height-32;
  if(this.position.x > width-33)         this.position.x = width-32;
  if(this.position.x < -this.width+32)   this.position.x = 0;
};
...
window.Game = (function(){
  ...
  var init = function(canvas) {
    ...
    ct.fillStyle = 'hsla(0,0%,50%,1)',
    
    player = new Player(32, 32, new Vector(width/4, height/2), new Vector(6, 4));
